# -*- coding: utf-8 -*-

# Form implementation generated from reading ui file 'settingswindow.ui'
#
# Created by: PyQt5 UI code generator 5.15.9
#
# WARNING: Any manual changes made to this file will be lost when pyuic5 is
# run again.  Do not edit this file unless you know what you are doing.


from PyQt5 import QtCore, QtGui, QtWidgets


class Ui_SettingsWindow(object):
    def setupUi(self, SettingsWindow):
        SettingsWindow.setObjectName("SettingsWindow")
        SettingsWindow.resize(900, 641)
        SettingsWindow.setMinimumSize(QtCore.QSize(900, 600))
        SettingsWindow.setMaximumSize(QtCore.QSize(900, 3000))
        self.centralwidget = QtWidgets.QWidget(SettingsWindow)
        self.centralwidget.setObjectName("centralwidget")
        self.gridLayout = QtWidgets.QGridLayout(self.centralwidget)
        self.gridLayout.setObjectName("gridLayout")
        self.label_6 = QtWidgets.QLabel(self.centralwidget)
        self.label_6.setObjectName("label_6")
        self.gridLayout.addWidget(self.label_6, 8, 6, 1, 1)
        self.b_createusermodel = QtWidgets.QPushButton(self.centralwidget)
        self.b_createusermodel.setObjectName("b_createusermodel")
        self.gridLayout.addWidget(self.b_createusermodel, 20, 4, 1, 1)
        self.l_device = QtWidgets.QLabel(self.centralwidget)
        self.l_device.setObjectName("l_device")
        self.gridLayout.addWidget(self.l_device, 8, 11, 1, 1)
        self.label_2 = QtWidgets.QLabel(self.centralwidget)
        self.label_2.setAlignment(QtCore.Qt.AlignRight|QtCore.Qt.AlignTrailing|QtCore.Qt.AlignVCenter)
        self.label_2.setObjectName("label_2")
        self.gridLayout.addWidget(self.label_2, 8, 10, 1, 1)
        self.label_5 = QtWidgets.QLabel(self.centralwidget)
        self.label_5.setObjectName("label_5")
        self.gridLayout.addWidget(self.label_5, 8, 4, 1, 1)
        self.label_3 = QtWidgets.QLabel(self.centralwidget)
        self.label_3.setAlignment(QtCore.Qt.AlignRight|QtCore.Qt.AlignTrailing|QtCore.Qt.AlignVCenter)
        self.label_3.setObjectName("label_3")
        self.gridLayout.addWidget(self.label_3, 14, 10, 1, 1)
        self.drp_class = QtWidgets.QComboBox(self.centralwidget)
        self.drp_class.setObjectName("drp_class")
        self.drp_class.addItem("")
        self.drp_class.addItem("")
        self.drp_class.addItem("")
        self.drp_class.addItem("")
        self.drp_class.addItem("")
        self.drp_class.addItem("")
        self.drp_class.addItem("")
        self.drp_class.addItem("")
        self.gridLayout.addWidget(self.drp_class, 14, 4, 1, 2)
        self.label_4 = QtWidgets.QLabel(self.centralwidget)
        self.label_4.setObjectName("label_4")
        self.gridLayout.addWidget(self.label_4, 8, 0, 1, 1)
        self.label = QtWidgets.QLabel(self.centralwidget)
        self.label.setAlignment(QtCore.Qt.AlignRight|QtCore.Qt.AlignTrailing|QtCore.Qt.AlignVCenter)
        self.label.setObjectName("label")
        self.gridLayout.addWidget(self.label, 20, 5, 1, 1)
        self.tb_currentmodel = QtWidgets.QLineEdit(self.centralwidget)
        self.tb_currentmodel.setObjectName("tb_currentmodel")
        self.gridLayout.addWidget(self.tb_currentmodel, 20, 6, 1, 1)
        self.buttonBox = QtWidgets.QDialogButtonBox(self.centralwidget)
        self.buttonBox.setStandardButtons(QtWidgets.QDialogButtonBox.Close|QtWidgets.QDialogButtonBox.Save)
        self.buttonBox.setObjectName("buttonBox")
        self.gridLayout.addWidget(self.buttonBox, 30, 11, 1, 1)
        self.b_getcurrentmodel = QtWidgets.QPushButton(self.centralwidget)
        self.b_getcurrentmodel.setObjectName("b_getcurrentmodel")
        self.gridLayout.addWidget(self.b_getcurrentmodel, 20, 11, 1, 1)
        self.l_mode = QtWidgets.QLabel(self.centralwidget)
        self.l_mode.setObjectName("l_mode")
        self.gridLayout.addWidget(self.l_mode, 14, 11, 1, 1)
        self.line = QtWidgets.QFrame(self.centralwidget)
        self.line.setFrameShape(QtWidgets.QFrame.VLine)
        self.line.setFrameShadow(QtWidgets.QFrame.Sunken)
        self.line.setObjectName("line")
        self.gridLayout.addWidget(self.line, 5, 0, 1, 1)
        self.table_widget = QtWidgets.QTableWidget(self.centralwidget)
        self.table_widget.setVerticalScrollBarPolicy(QtCore.Qt.ScrollBarAlwaysOn)
        self.table_widget.setAlternatingRowColors(True)
        self.table_widget.setSelectionBehavior(QtWidgets.QAbstractItemView.SelectItems)
        self.table_widget.setWordWrap(False)
        self.table_widget.setCornerButtonEnabled(False)
        self.table_widget.setObjectName("table_widget")
        self.table_widget.setColumnCount(5)
        self.table_widget.setRowCount(0)
        item = QtWidgets.QTableWidgetItem()
        self.table_widget.setHorizontalHeaderItem(0, item)
        item = QtWidgets.QTableWidgetItem()
        self.table_widget.setHorizontalHeaderItem(1, item)
        item = QtWidgets.QTableWidgetItem()
        self.table_widget.setHorizontalHeaderItem(2, item)
        item = QtWidgets.QTableWidgetItem()
        self.table_widget.setHorizontalHeaderItem(3, item)
        item = QtWidgets.QTableWidgetItem()
        self.table_widget.setHorizontalHeaderItem(4, item)
        self.table_widget.verticalHeader().setVisible(False)
        self.gridLayout.addWidget(self.table_widget, 18, 0, 1, 13)
        self.cb_show_inherited = QtWidgets.QCheckBox(self.centralwidget)
        self.cb_show_inherited.setChecked(True)
        self.cb_show_inherited.setObjectName("cb_show_inherited")
        self.gridLayout.addWidget(self.cb_show_inherited, 20, 0, 1, 1)
        self.drp_models = QtWidgets.QComboBox(self.centralwidget)
        self.drp_models.setEditable(False)
        self.drp_models.setObjectName("drp_models")
        self.drp_models.addItem("")
        self.gridLayout.addWidget(self.drp_models, 14, 6, 1, 1)
        self.drp_sim = QtWidgets.QComboBox(self.centralwidget)
        self.drp_sim.setEditable(False)
        self.drp_sim.setObjectName("drp_sim")
        self.drp_sim.addItem("")
        self.drp_sim.addItem("")
        self.drp_sim.addItem("")
        self.drp_sim.addItem("")
        self.gridLayout.addWidget(self.drp_sim, 14, 0, 1, 3)
        SettingsWindow.setCentralWidget(self.centralwidget)

        self.retranslateUi(SettingsWindow)
        QtCore.QMetaObject.connectSlotsByName(SettingsWindow)

    def retranslateUi(self, SettingsWindow):
        _translate = QtCore.QCoreApplication.translate
        SettingsWindow.setWindowTitle(_translate("SettingsWindow", "TelemFFB Settings Manager"))
        self.label_6.setText(_translate("SettingsWindow", "Model"))
        self.b_createusermodel.setText(_translate("SettingsWindow", "Create Model Setting"))
        self.l_device.setText(_translate("SettingsWindow", "unknown"))
        self.label_2.setText(_translate("SettingsWindow", "Device:"))
        self.label_5.setText(_translate("SettingsWindow", "Class"))
        self.label_3.setText(_translate("SettingsWindow", "Edit Mode:"))
        self.drp_class.setItemText(0, _translate("SettingsWindow", "(None - Global)"))
        self.drp_class.setItemText(1, _translate("SettingsWindow", "(None - Sim Default)"))
        self.drp_class.setItemText(2, _translate("SettingsWindow", "PropellerAircraft"))
        self.drp_class.setItemText(3, _translate("SettingsWindow", "TurbopropAircraft"))
        self.drp_class.setItemText(4, _translate("SettingsWindow", "JetAircraft"))
        self.drp_class.setItemText(5, _translate("SettingsWindow", "GliderAircraft"))
        self.drp_class.setItemText(6, _translate("SettingsWindow", "Helicopter"))
        self.drp_class.setItemText(7, _translate("SettingsWindow", "HPGHelicopter"))
        self.label_4.setText(_translate("SettingsWindow", "Simulator"))
        self.label.setText(_translate("SettingsWindow", "Current Aircraft:"))
        self.tb_currentmodel.setText(_translate("SettingsWindow", "Wilga 80X"))
        self.b_getcurrentmodel.setText(_translate("SettingsWindow", "Get Current Model"))
        self.l_mode.setText(_translate("SettingsWindow", "Model"))
        self.table_widget.setSortingEnabled(True)
        item = self.table_widget.horizontalHeaderItem(0)
        item.setText(_translate("SettingsWindow", "New Column"))
        item = self.table_widget.horizontalHeaderItem(1)
        item.setText(_translate("SettingsWindow", "New Column"))
        item = self.table_widget.horizontalHeaderItem(2)
        item.setText(_translate("SettingsWindow", "New Column"))
        item = self.table_widget.horizontalHeaderItem(3)
        item.setText(_translate("SettingsWindow", "New Column"))
        item = self.table_widget.horizontalHeaderItem(4)
        item.setText(_translate("SettingsWindow", "New Column"))
        self.cb_show_inherited.setText(_translate("SettingsWindow", "Show Inherited"))
        self.drp_models.setItemText(0, _translate("SettingsWindow", "(None - Class Default)"))
        self.drp_sim.setItemText(0, _translate("SettingsWindow", "Global"))
        self.drp_sim.setItemText(1, _translate("SettingsWindow", "DCS"))
        self.drp_sim.setItemText(2, _translate("SettingsWindow", "IL2"))
        self.drp_sim.setItemText(3, _translate("SettingsWindow", "MSFS"))
