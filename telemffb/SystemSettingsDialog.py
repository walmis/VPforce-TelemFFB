...
#: The Integrations toggle explains a separately sold product, so it has
#: more to say than a one-line hint.  Kept here rather than in the .ui:
#: Designer stores a tooltip as a single escaped line, which at this
#: length cannot be read or edited by hand in either the .ui or the
#: generated module.
#:
#: Rich text on purpose - Qt wraps markup and never wraps plain text, so
#: a tooltip this long would otherwise render as one line off the screen.
DINPUT_TOOLTIP = (
    # "DirectInput Force Feedback" is the qualifier, not padding: plenty of
    # force feedback hardware exposes its forces through a proprietary API
    # or not at all, and DirectLink cannot drive those.  Capitalized as
    # Microsoft's own DirectInput SDK documentation writes it.
    "<p><b>DirectLink:</b> Extends TelemFFB to support DirectInput "
    "Force Feedback enabled devices from any manufacturer.</p>"

    "<p>Such devices are listed in the device selectors marked "
    "<b>[DI]</b>, and can be assigned to any role - joystick, pedals, "
    "collective, or trim wheel.  The device then receives the full "
    "effect suite generated by TelemFFB.</p>"

    "<p><b>DirectLink</b> is a separate product and must be installed "
    "before this can be enabled.</p>"

    "<p>The <b>DirectInput Tap</b> (bundled with TelemFFB) must also be "
    "enabled and configured for these devices to work seamlessly with "
    "Force Feedback native sims (<b>DCS</b>, <b>IL-2 Series</b> and "
    "<b>Falcon BMS</b>).  Support for <b>MSFS</b> and <b>X-Plane</b> does "
    "not require the DirectInput Tap configuration.</p>"
)
...
class SystemSettingsDialog(QDialog, Ui_SystemDialog):
    def __init__(self, parent=None,):
        ...
        self.cb_enable_dinput.setToolTip(DINPUT_TOOLTIP)
    ...
    def toggle_dinput_support(self):
        ...
        if self.cb_enable_dinput.isChecked():
            ...
            if not available:
                QMessageBox.warning(self, "DirectLink Not Available", reason)
