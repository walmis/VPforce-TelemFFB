...
from, and the moved active profile stays active. When their pattern claims exactly what
...
built-in. Every setting in effect stays exactly as it was; what changes is that the two
...
sets, and the last what would be in effect afterward. Each column is headed with the match
